...
public class MPUnit : MonoBehaviour
{
    ...
    void Update()
    {
        if (GameObject.Find("MPController").GetComponent<MPGameController>().startGame && alive == true)
        {
            ...
            else if (enemyDistance > 26f && enemyDistance < 35f && unitStats.range > 1)
            {
                ...
                newBattleIcon.transform.position = this.transform.position;
                newBattleIcon.transform.rotation = Quaternion.Euler(Vector3.forward * (angle + offset));
                newBattleIcon.transform.localScale = new Vector3(1.78f, 1.78f, -1f);
                attacking = true;
                if (Time.time > elapsedTime)
                {
                    Attack(enemyTarget);
            ...
            {
                if (GameObject.Find("MPController").GetComponent<MPGameController>().selectedHexCell)
                {
                    if (GameObject.Find("MPController").GetComponent<MPGameController>().selectedHexCell.full == gameObject)
                    {
                        foreach (HexCell neighbor in GameObject.Find("MPController").GetComponent<MPGameController>().selectedHexCell.neighbors)
                        {
                            if (neighbor)
                            {
                        ...
                        GameObject.Find("MPController").GetComponent<MPGameController>().selectedHexCell.full = null;
                        GameObject.Find("MPController").GetComponent<MPGameController>().selectedHexCell = null;
                    }
                }
                if (team == "A")
                {
                    GameObject.Find("MPController").GetComponent<MPGameController>().redTeamCount--;
                }
                else if (team == "B")
                {
                GameObject.Find("MPController").GetComponent<MPGameController>().blueTeamCount--;
    ...
    public virtual void Attack(Unit enemyTarget)
    {
        ...
        if (enemyTarget.GetComponent<Unit>().unitStats.currentStrength > 0)
        {
            ...
            {
                baseDamage = Math.Max(0, (Random.Range(0,7) - enemyTarget.GetComponent<Unit>().position.GetComponent<HexCell>().attackerPenalty + unitStats.attackDamage + counterAttack) * 100 / (100 + 100 * enemyTarget.GetComponent<Unit>().unitStats.pierceArmor/unitStats.attackDamage) * unitStats.currentStrength / unitStats.strength);
            }
            else
            {
                baseDamage = Math.Max(0, (Random.Range(0,7) - enemyTarget.GetComponent<Unit>().position.GetComponent<HexCell>().attackerPenalty + unitStats.attackDamage  + counterAttack) * 100 / (100 + 100 * enemyTarget.GetComponent<Unit>().unitStats.meeleArmor/unitStats.attackDamage) * unitStats.currentStrength / unitStats.strength);
            }
            float incidence = Random.Range(nightDebuff * (1.2f-(longRange/5)) * (unitStats.accuracy - enemyTarget.GetComponent<Unit>().unitStats.dodgeChange), unitStats.accuracy - enemyTarget.GetComponent<Unit>().unitStats.dodgeChange);
            float hillBonus = 1f;
